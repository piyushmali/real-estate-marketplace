...
// Simple property metadata for testing
const propertyMetadata = {
  property_id: "TestTransfer123",
  price: 1000000,
  metadata_uri: "https://example.com/metadata/test123.json",
...
async function generateSimpleTransfer() {
  try {
    ...
    // Get blockhash from backend API
    ...
    // Add a simple transfer instruction (just send 0.001 SOL to self)
    const transferInstruction = SystemProgram.transfer({
      fromPubkey: signer.publicKey,
      toPubkey: signer.publicKey, // sending to self
    ...
    fs.writeFileSync('transfer_payload.json', JSON.stringify(requestPayload, null, 2));
    console.log('Request payload saved to transfer_payload.json');
